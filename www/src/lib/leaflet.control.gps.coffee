L.Control.ShowLocation = L.Control.extend(
  options:
    position: 'topright'
    text: ''
    iconUrl: 'dist/images/location_finder.png'
  ...
  toggleLocationTracking: (e) ->
    ...
    if @showLocation
      @trackingToggler.setAttribute('src', 'dist/images/show.png')
      @startTracking()
    else
      @trackingToggler.setAttribute('src', 'dist/images/hide.png')
      @stopTracking()
  ...
  drawLocation: (position) ->
    ...
    GpsIcon = L.Icon.extend(
      options:
        iconUrl: 'dist/images/gps-marker.png'
        iconSize: [16, 16]
    )
